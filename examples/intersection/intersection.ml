...
let new_val () =
  ...
  and c2 = Circle.make d radius in
  (c1, c2, l)
...
let work2 c =
  let top = Line.make_y 0. size_y and right = Line.make_x size_x in
  let a = Circle.intersect_line c Line.x_axis
  and b = Circle.intersect_line c Line.y_axis
  and c = Circle.intersect_line c top
  and d = Circle.intersect_line c right in
  List.concat [ a; b; c; d ]

let frame (c1, c2, l) =
  Drawing.draw_string 25 675 "Press 'r' to generate new circles and a new line"
    Graphics.black;
  Drawing.draw_circle c1 Graphics.red;
  Drawing.draw_circle c2 Graphics.red;
  Drawing.draw_line l Graphics.blue;
  List.iter
    (fun e -> Drawing.fill_circle (Circle.make e 5.) Graphics.black)
    (work (c1, c2, l));
  List.iter
    (fun e -> Drawing.fill_circle (Circle.make e 5.) Graphics.blue)
    (work2 c1);
  List.iter
    (fun e -> Drawing.fill_circle (Circle.make e 5.) Graphics.red)
    (work2 c2)
