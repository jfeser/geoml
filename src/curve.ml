module Quadratic = struct
  ...
  let equation (p0, p1, p2) t =
    ...
    let open Point in
    if t <= 1. && t >= 0. then
      let x = b p0.x p1.x p2.x and y = b p0.y p1.y p2.y in
      make x y
  ...
  let points b nb =
    ...
    if Int.(nb <= 0) then []
      ...
      let res = ref [] and cur = ref 0. and step = 1. /. of_int nb in
      while !cur <= 1. do
...
module Cubic = struct
  ...
  let equation (p0, p1, p2, p3) t =
    ...
    let open Point in
    if t <= 1. && t >= 0. then
      let m_t = (1. -. t) *. (1. -. t) and t_2 = t *. t in
      let x =
        (p0.x *. m_t *. (1. -. t))
        +. (3. *. p1.x *. t *. m_t)
        +. (3. *. p2.x *. t_2 *. (1. -. t))
  ...
  let of_points pts =
    let rec aux res pts =
      ...
      | x :: y :: _ ->
          List.rev
            ( make x
                (Point.barycenter [ (x, 2.); (y, 1.) ])
                (Point.barycenter [ (x, 1.); (y, 2.) ])
                y
            :: res )
...
module BSpline = struct
  ...
  let make_eq pts nb_knots =
    let step = 1. /. Float.of_int (nb_knots - 1) and tmp = ref 0. in
    let next () =
      let res = !tmp in
      tmp := !tmp +. step;
      res
    in
    let k = Array.create ~len:nb_knots 0. |> Array.map ~f:(fun _ -> next ()) in
    ...
    if t < 0. || t > 1. then failwith "t must be in [0. ; 1.]"
      ...
      and bspl j d x =
        if Int.(j < 0 || j >= d) then 0.
          ...
          if knots.(j - 1) <= x && x <= knots.(j) then 1. else 0.
        else if knots.(j - 1) = knots.(j + d) then 0.
      ...
      and n = List.length control in
      let x =
        Math.float_sum 1 n (fun i ->
