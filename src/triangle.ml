...
let print fmt (a, b, c) =
  Caml.Format.fprintf fmt "(%a, %a, %a)" Point.print a Point.print b Point.print
    c
...
let intersects (s1 : t) (s2 : t) =
  ...
  tri_exists
    (fun s ->
      Segment.intersects d s || Segment.intersects e s || Segment.intersects f s)
    (a, b, c)
...
let intersect_line (t : t) (l : Line.t) =
  ...
  List.fold_left
    ~f:(fun acc s ->
      match Segment.intersect_line s l with None -> acc | Some p -> p :: acc)
    ~init:[] [ a; b; c ]
