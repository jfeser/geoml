...
let circumscribed p1 p2 p3 =
  let b1 = Line.point_bissection p1 p2 and b2 = Line.point_bissection p2 p3 in
...
let bounding (pts : Point.t list) : t =
  ...
  let rec mindisk l circle set pts =
    ...
    | [] -> circle
    | h :: tl
      when contains circle h || List.mem ~equal:[%compare.equal: Point.t] set h
      ->
    ...
    | h :: _ ->
        ...
        List.filter
          ~f:(fun e ->
            List.mem ~equal:[%compare.equal: Point.t] new_set e |> not)
          l
...
let random_point (c : t) : Point.t =
  let open Float in
  let theta = Random.float pi *. 2.
  and r = Random.float (c.radius *. c.radius) |> sqrt in
