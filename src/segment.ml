...
let proj_x ((a, b) : t) =
  let open Point in
  if a.x > b.x then (b.x, a.x) else (a.x, b.x)

let proj_y ((a, b) : t) =
  let open Point in
  if a.y > b.y then (b.y, a.y) else (a.y, b.y)
...
let intersects ((a1, b1) as s1 : t) ((a2, b2) as s2 : t) =
  ...
  try
    ...
    sq_distance a1 p <= sqd
    && sq_distance b1 p <= sqd
    && sq_distance a2 p <= sqd2
    && sq_distance b2 p <= sqd2
  with Line.Error (Line.Parallel _) -> false
...
let intersection ((a1, b1) as s1 : t) ((a2, b2) as s2 : t) =
  ...
  with Line.Error (Line.Parallel _) -> None

let intersect_line ((p1, p2) as s : t) l =
  let open Point in
  try
    let p = Line.intersection l (to_line s) and sqd = sq_distance p1 p2 in
    if sq_distance p1 p <= sqd && sq_distance p2 p <= sqd then Some p else None
  with Line.Error (Line.Parallel _) -> None
