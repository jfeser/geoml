...
let print fmt =
  ...
  function
  | Y (a, b) -> fprintf fmt "y=%fx +. %f" a b | X c -> fprintf fmt "x=%f" c
...
let print_error fmt e =
  ...
  match e with
  | Parallel (l1, l2) ->
      fprintf fmt "Bad arguments : parallel lines %a and %a" print l1 print l2
  | Same_coordinates p ->
      fprintf fmt "Bad arguments : same coordinates %a" Point.print p

let make a b c = if b = 0. then X (-.c /. a) else Y (-.a /. b, -.c /. b)
...
let of_points (p1 : Point.t) (p2 : Point.t) =
  let open Point in
  if [%compare.equal: Point.t] p1 p2 then raise (Error (Same_coordinates p1))
...
let scale_y (l : t) f = match l with Y (a, b) -> Y (a *. f, b *. f) | _ -> l
...
let arbitrary_point l =
  match l with X c -> Point.make c 0. | Y (_, b) -> Point.make 0. b
